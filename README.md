...
PopTab is a lightweight, open-source Chromium extension that turns the current browser tab into a clean desktop window—without the tab strip, address bar, bookmarks bar, navigation buttons, or extension controls.
...
PopTab provides a middle ground: a toolbar-free browser view that still behaves like a regular desktop window.
...
When you enter clean-window mode, PopTab records the active tab's source browser window, logical tab position, pinned state, and relevant window geometry.

It then moves that same live tab into a Chromium popup window.

When you exit, PopTab moves the same tab back to its source browser window and restores its logical position and pinned state.
...
PopTab return tickets are valid only for the current browser session.

After a complete browser restart, Chromium may restore a previous clean popup without its original PopTab return ticket. In that situation, pressing `Ctrl+Shift+F` converts the same restored live tab back into a normal browser window with the browser interface available again.

PopTab does not attempt to guess the tab's previous source window or tab index across a full browser restart.

## Install from source

Until packaged releases and browser-store installation are available, PopTab can be loaded directly from the repository.
